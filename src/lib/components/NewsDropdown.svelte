...
<div>
  ...
  <Dropdown class="w-full">
    <div slot="header" class="p-3">
      <Search size="md" bind:value={searchTerm} placeholder="Search news by title..." debounce={300} />
    </div>
    ...
    {#each newsItems as item}
      <DropdownItem class="flex items-center justify-between" on:click={(event) => handleSelection(item, event)}>
        {item.title}
