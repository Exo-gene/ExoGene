...
import type {
  ...
  SubCategoryLanguageModel,
  SubCategoryModel,
} from "../models/subCategoryModel";

const createSubCategoriesStore = () => {
  const { subscribe, set, update } = writable<SubCategoryModel[]>([]);

  return {
    subscribe,
    set: (subCategories: SubCategoryModel[]) => {
      set(subCategories);
    },
...
export const subCategoriesStore = createSubCategoriesStore();
