<script lang="ts">
  ...
  import type { FormData, MagazineLanguageModelToUpdate } from "../../../../models/magazineModel";
  ...
  const id = +$page.params.magazineId; 
   onMount(async () => {
  isLoading = true;
  let { data: magazineData, error } = await supabase.rpc(
    "get_magazine_by_id",
    {
      input_magazine_id: id,
    }
  );
  ...
  if (!magazineData || magazineData.length === 0) {
    console.log("No magazine data found.");
    isLoading = false;
    return;
  }

  const firstMagazineData = magazineData[0];
  firstMagazineData.magazine_translations.forEach((translation: {
    ...
    if (formData[translation.language]) {
      ...
      formData[translation.language].pdfFileName = translation.pdfFile.split('/').pop();
    }
  });
 ...
 async function formSubmit() {
  ...
  const magazineLanguageData: MagazineLanguageModelToUpdate[] = [];
  ...
  for (const language of languages) {
    ...
    if (formData[language].number === undefined || formData[language].number === null) {
    ...
    if (isValid) {
      try {
        ...
        if (formData[language].pdfFile instanceof File) {
          imagePath = await uploadFile(formData[language].pdfFile as File, language);
        }
        ...
        console.error("Error during magazine update:", error);
        showAlert = true;
        alertMessage = "There was an error updating the magazine. Please try again.";
        isLoading = false;
        return;
      }
    }
  ...
  try {
    await magazineStore.updateMagazineData(magazineObject, magazineLanguageData, supabase);
                  ...
                  <div class="mb-4">
                    ...
                    {#if formData[language].pdfFile}
                      <span class="block mt-2 text-sm text-gray-700">
                        Selected File: <a href={getObjectUrl(formData[language].pdfFile)} target="_blank" class="text-orange-500 underline">{formData[language].pdfFileName}</a>
                      </span>
                    {/if}
...
{#if showToast}
  <Toast message="Magazine has been updated successfully" type="success" />
{/if}
