<script lang="ts">
 import { InfoCircleSolid } from 'flowbite-svelte-icons';
  ...
  const sampleTypeId = +$page.params.sampleTypeId;
  ...
  async function formSubmit() {
    ...
    try {
      ...
      await sampleTypeStore.updateSampleTypeData(sampleTypeObject, sampleTypeId, supabase);

      showToast = true;
      setTimeout(() => {
        showToast = false;
        goto("/dashboard/sampleType");
      }, 3000);
    } catch (error) {
      console.error("Error during sampleType update:", error);
    } finally {
      isLoading = false;
    }
  }
...
{#if showToast}
  <Toast message="SampleType data has been updated successfully" type="success" />
{/if}
