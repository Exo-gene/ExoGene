<script lang="ts">
	import { toLocaleDate } from '$lib/utils/dateTimeFormat';
  ...
  async function loadStoreData() {
    try {
      ...
      registered_date = toLocaleDate(data.registered_date);
      expiration_date = toLocaleDate(data.expiration_date);
  ...
  async function formSubmit() {
    ...
    try {
      ...
      if (storeId) {
        await storeDataStore.updateStoreData(storeObject, storeId, supabase);
      } 
...
<div class="max-w-screen-2xl mx-auto py-10">
  <div class="w-full flex items-center justify-between py-4">
    ...
    <h1 class="font-bold text-center flex-grow" style="color: var(--titleColor);">
      {storeId ? "Update Store Data" : "Create Store Data"}
    </h1>
  </div>
  {#if showErrorAlert}
...
{#if showToast}
  <Toast message={storeId ? "Store data updated successfully!" : "New store has been inserted successfully!"} type="success" />
{/if}
